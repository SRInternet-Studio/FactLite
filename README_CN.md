...
## 📊 基准测试 (AA-Omniscience Benchmark)

为了严格量化 FactLite 缓解幻觉的能力，我们在 **AA-Omniscience Benchmark（全知基准）** 上进行了测试。这是一个以难度极高著称的数据集，包含高度垂直的专业领域问题（金融、法律、医疗等）。

AA-Omniscience 的评分标准对大模型的幻觉行为有极其严厉的惩罚：`全知指数 (OI) = 正确率(%) - 错误率(%)`。而模型选择弃权（承认不知道）则不扣分（0 惩罚）。

**测试环境配置：**
*   **基础模型：** DeepSeek V4 Flash
*   **样本量：** 201 道事实性问题 (来自 [AA-Omniscience 公开数据集](https://huggingface.co/datasets/ArtificialAnalysis/AA-Omniscience-Public)中的随机抽取)
*   **FactLite 配置：** 使用 `@verify` 装饰器，挂载 `Web_LLMJudge`（自动选择搜索后端）以及 `ReturnSafeMessage` 安全兜底策略。

### 📈 性能评估结果

| 指标 | Baseline | 接入 FactLite | 变化幅度 |
| :--- | :--- | :--- | :--- |
| **✅ 正确** | 36.0% | 32.0% | -4.0% |
| **❌ 错误/致命幻觉** | **63.5%** | **16.0%** | **📉 -47.5%** |
| **🛡️ 弃权/安全兜底** | 0.5% | 52.0% | 📈 +51.5% |
| **🏆 Omniscience Index** | **-27.50** | **+16.00** | **🚀 +43.50 分** |

你可以在 [aa_omniscience_eval_report.csv](aa_omniscience_eval_report.csv) 测试报告中查看每一道题的回答结果。

### 🔬 统计学显著性证明 (P-Value)

为了确保我们观察到的幻觉率骤降并非偶然现象，我们进行了统计学假设检验（针对配对分类数据的**麦克尼马尔检验**）。

*   **原假设 ($H_0$)**：FactLite 对降低大模型的幻觉率没有实际影响。
*   **检验结果**：高达 47.5% 的绝对降幅使得 Z-score 接近 10.0，计算得出 **P-Value (概率) < 0.0001**。
*   **最终裁决**：我们以 **>99.99% 的置信度**拒绝原假设。幻觉的减少具有极高的统计学显著性，这不仅证明了 FactLite 架构的防御能力是高度确定且可复现的，更排除了统计噪音的干扰。

**总结：** 
通过智能检索外部事实，并在缺乏确凿证据时强制执行“快速失败”的兜底拦截，FactLite 成功斩断了近 50% 的致命幻觉，从根本上提升了 AI 在企业级生产环境中的可信度与可靠性。
